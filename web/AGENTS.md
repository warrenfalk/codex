...
Default ports are UI `4200`, relay `4201`, and backend websocket `ws://127.0.0.1:4222`. Override with `CODEX_WEB_UI_PORT`, `CODEX_WEB_PROXY_PORT`, and `CODEX_WEB_BACKEND_WS_URL`. Do not add persistence, auth, or bridge protocol complexity unless the task explicitly calls for it.
