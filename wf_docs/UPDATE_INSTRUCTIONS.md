...
If the Python formatter portion of `just fmt` fails on NixOS because `uv`
downloaded a generic Linux `ruff` wheel, run the Python formatter passes with a
Nix-provided Ruff from the repo root instead of committing formatter failures
or unrelated churn:

```bash
nix shell nixpkgs#ruff -c ruff check --config sdk/python/pyproject.toml --fix --fix-only sdk/python
nix shell nixpkgs#ruff -c ruff format --config sdk/python/pyproject.toml sdk/python
nix shell nixpkgs#ruff -c ruff format --config scripts/pyproject.toml scripts
```
