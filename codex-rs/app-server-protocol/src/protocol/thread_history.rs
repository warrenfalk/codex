...
impl ThreadHistoryBuilder {
    ...
    fn handle_thread_rollback(&mut self, payload: &ThreadRolledBackEvent) {
        ...
        let removed_turn_ids = if let Some(&(turn_idx, item_idx)) = if n >= user_positions.len() {
            ...
            let explicit_turn_positions = self
                .turns
                .iter()
                .enumerate()
                .filter_map(|(turn_idx, turn)| turn.opened_explicitly.then_some(turn_idx))
                .collect::<Vec<_>>();
            let Some(&turn_idx) = (if n >= explicit_turn_positions.len() {
                explicit_turn_positions.first()
            } else {
                explicit_turn_positions.get(explicit_turn_positions.len() - n)
            }) else {
                return;
            };
            let removed_turn_ids = self.turns[turn_idx..]
                .iter()
                .map(|turn| turn.id.clone())
                .collect::<Vec<_>>();
            self.turns.truncate(turn_idx);
            removed_turn_ids
        };
...
mod tests {
    ...
    fn thread_rollback_ignores_note_only_turns_as_boundaries() {
        ...
        let note = |note: &str| {
            EventMsg::NoteToSelf(NoteToSelfEvent {
                note: note.into(),
                ..Default::default()
            })
        };
