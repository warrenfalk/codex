...
#[tokio::test]
async fn firehose_observes_turn_completion_after_last_thread_subscriber_leaves() -> Result<()> {
    ...
    let mut owner = connect_websocket(bind_addr).await?;
    let mut firehose = connect_websocket(bind_addr).await?;
    send_initialize_request(&mut owner, /*id*/ 1, "firehose_departing_owner").await?;
    read_response_for_id(&mut owner, /*id*/ 1).await?;
    send_initialize_request(&mut firehose, /*id*/ 2, "firehose_remaining_observer").await?;
    read_response_for_id(&mut firehose, /*id*/ 2).await?;
    send_request(
        &mut firehose,
        "event/firehose",
    ...
    send_request(
        ...
        Some(serde_json::to_value(TurnStartParams {
            ...
            input: vec![V2UserInput::Text {
                text: "finish after I unsubscribe".to_string(),
    ...
    .await?;
    let unsubscribe = read_response_for_id(&mut owner, /*id*/ 6).await?;
    assert_eq!(
        to_response::<ThreadUnsubscribeResponse>(unsubscribe)?,
    ...
    release_response
        .send(())
        .expect("response should still be waiting for the gate");

    let completed = read_notification_for_method(&mut firehose, "turn/completed").await?;
    let completed: TurnCompletedNotification = serde_json::from_value(
        completed
            .params
            .context("turn/completed params should be present")?,
    )?;
    assert_eq!(
        (completed.thread_id, completed.turn.status),
        (thread_id, TurnStatus::Completed)
    );
    assert!(
        timeout(
        ...
        .await
        .is_err(),
        "the unsubscribed ordinary connection should not receive turn/completed"
    );
