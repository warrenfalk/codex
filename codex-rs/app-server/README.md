...
- `turn/startModelOnly` — experimental; start an idle turn that retains the target thread's model-visible history but exposes no tools and skips hooks, skill/plugin injection, memory startup work, and automatic title generation. It accepts `threadId`, `input`, `model`, optional `effort`, and optional `outputSchema`; responds with the initial `turn` and emits the usual turn/item notifications. The input and output are ordinary history on the target thread, so clients that need a hidden transformation should use a separate ephemeral thread. Parent-owned Multi-Agent V2 subagents reject direct turns.
