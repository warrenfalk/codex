...
impl OutboundConnectionState {
    ...
    pub(crate) fn new(
        ...
        Self::new_with_session(
            ...
            Arc::new(ConnectionSessionState::new(ConnectionOrigin::Stdio)),
            disconnect_sender,
        )
    }
...
fn firehose_observed_message(message: &OutgoingMessage) -> Option<OutgoingMessage> {
    match message {
        OutgoingMessage::AppServerNotification(envelope) => {
            Some(OutgoingMessage::AppServerNotification(envelope.clone()))
        }
        OutgoingMessage::Request(request) => {
            Some(crate::outgoing_message::timestamped_server_notification(
