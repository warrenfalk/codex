...
pub(crate) fn render_limit_progress_bar(
    percent_remaining: f64,
    segments: usize,
    time_remaining_percent: Option<f64>,
) -> String {
    ...
    let mut bar = vec![STATUS_LIMIT_BAR_EMPTY; segments];
    ...
    if let Some(time_remaining_percent) = time_remaining_percent {
        ...
        let marker_index = ((ratio * segments.saturating_sub(1) as f64).round() as usize)
