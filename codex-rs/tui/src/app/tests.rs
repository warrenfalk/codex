...
async fn clean_scrollback_syncs_transcript_overlay_to_filtered_and_full_cells() {
    ...
    let filtered_count = match app.overlay.as_ref() {
    ...
    let restored_count = match app.overlay.as_ref() {
        Some(Overlay::Transcript(t)) => t.committed_cell_count(),
        _ => panic!("expected transcript overlay"),
    };
    assert_eq!(restored_count, app.transcript_cells.len());
}
...
#[tokio::test]
async fn backtrack_overlay_copy_shortcut_copies_agent_message_before_selected_prompt() {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let thread_id = ThreadId::new();
    app.chat_widget.handle_thread_session(test_thread_session(
        thread_id,
    ...
    let mut copied = None;
    app.copy_agent_message_before_selected_prompt_with(|input| {
        copied = Some(input.to_string());
        Ok(Some(crate::clipboard_copy::ClipboardLease::test()))
    });

    assert_eq!(copied, Some("previous **response**".to_string()));
    let status_cell = match app_event_rx.try_recv() {
        Ok(AppEvent::InsertHistoryCell(cell)) => cell,
        other => panic!("expected copy status history cell, got {other:?}"),
    };
    let rendered = lines_to_single_string(&status_cell.display_lines(/*width*/ 80));
    assert!(
        rendered.contains("Copied previous response to clipboard"),
        "expected previous-response copy status, got {rendered:?}"
    );
}
...
async fn replace_chat_widget_preserves_connected_mode_footer_state() {
    ...
    app.app_server_footer_state = Some(crate::chatwidget::ConnectedModeFooterState::Connected);
    ...
    app.app_server_footer_state = Some(crate::chatwidget::ConnectedModeFooterState::Disconnected);
