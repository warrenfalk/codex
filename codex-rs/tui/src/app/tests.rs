...
async fn side_close_shortcut_preserves_composer_and_active_popup_behavior() {
    ...
    app.side_threads
        .insert(side_thread_id, SideThreadState::new(parent_thread_id));
    app.chat_widget
        .restore_user_message_to_composer(crate::chatwidget::UserMessage::from("draft"));
...
async fn backtrack_overlay_copy_shortcut_copies_agent_message_before_selected_prompt() {
    ...
    let rendered = lines_to_single_string(&status_cell.display_lines(/*width*/ 80));
    assert_app_snapshot!(
        "backtrack_overlay_copy_shortcut_copies_agent_message_before_selected_prompt",
        rendered
    );
}
...
async fn replace_chat_widget_preserves_connected_mode_footer_state() {
    ...
    let mut app = make_test_app().await;
    app.local_settings.tui.status_line = Some(vec!["current-dir".to_string()]);
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        requires_openai_auth: true,
        local_settings: app.local_settings.clone(),
        config: app.config.clone(),
        frame_requester: crate::tui::FrameRequester::test_dummy(),
        app_event_tx: app.app_event_tx.clone(),
        workspace_command_runner: None,
