...
pub(super) struct PendingSideSummary {
    ...
    destination: SideSummaryDestination,
    turn: PendingSideSummaryTurn,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum SideSummaryDestination {
...
impl App {
    pub(super) async fn handle_side_conversation_close_choice(
        ...
        match choice {
            ...
            | SideConversationCloseChoice::SummarizeForMe => {
                ...
                let destination = match choice {
                    SideConversationCloseChoice::Summarize => {
                        SideSummaryDestination::ParentAssistantMessage
                    }
                    SideConversationCloseChoice::SummarizeForMe => {
                        SideSummaryDestination::ParentNoteToSelf
                    }
                    SideConversationCloseChoice::Cancel | SideConversationCloseChoice::Leave => {
                        unreachable!("handled above")
                    }
                };
                self.start_side_summary(app_server, side_thread_id, parent_thread_id, destination)
                    .await?;
            }
        }
        Ok(())
        ...
        match pending.destination {
            ...
            SideSummaryDestination::ParentNoteToSelf => {
                if let Err(err) = app_server
                    .thread_note_create(
                        pending.parent_thread_id,
                        Self::formatted_side_summary(summary),
                    )
                    .await
                {
                    self.chat_widget.add_error_message(format!(
                        "Failed to save side conversation summary as a note to self; the side conversation is still open: {err}"
                    ));
                    self.keep_side_thread_visible_after_cleanup_failure(
                        tui,
                        app_server,
    ...
    fn formatted_side_summary(summary: &str) -> String {
        format!("{SIDE_SUMMARY_HEADING}\n\n{}", summary.trim())
    }
...
mod tests {
    ...
    #[test]
    fn formatted_side_summary_is_note_to_self_body() {
        assert_eq!(
