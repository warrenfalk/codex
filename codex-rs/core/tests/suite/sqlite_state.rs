...
async fn first_turn_auto_generates_thread_title() -> Result<()> {
    ...
    for _ in 0..100 {
        metadata = db.get_thread(thread_id).await?;
        indexed_name = find_thread_name_by_id(test.codex_home_path(), &thread_id).await?;
        if metadata
            .as_ref()
            .is_some_and(|metadata| metadata.title == "Parser bug fix")
            && indexed_name.as_deref() == Some("Parser bug fix")
        {
            break;
        }
        tokio::time::sleep(Duration::from_millis(25)).await;
    ...
    assert_eq!(indexed_name, Some("Parser bug fix".to_string()));
