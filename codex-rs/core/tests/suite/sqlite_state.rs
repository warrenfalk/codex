...
async fn first_turn_auto_generates_thread_title() -> Result<()> {
    ...
    wait_for_event_match(&test.codex, |event| match event {
        EventMsg::ThreadNameUpdated(event)
            if event.thread_id == thread_id
                && event.thread_name.as_deref() == Some("Parser bug fix") =>
        {
            Some(())
        }
        _ => None,
    ...
    // ResponseMock records before custom matchers run, so count only HTTP requests
    // matching each title/turn predicate rather than every attempted mock match.
    let requests = server.received_requests().await.expect("recorded requests");
    let initial_requests: Vec<_> = requests
        .iter()
        .filter(|request| {
            is_auto_thread_title_request(request)
                && !request_body_contains(request, "Latest assistant reply")
        ...
        .collect();
    let turn_request_count = requests
        .iter()
        .filter(|request| {
            !is_auto_thread_title_request(request)
                && request_body_contains(request, "please fix the parser bug in the lexer")
        ...
        .count();
    let revision_requests: Vec<_> = requests
        .iter()
        .filter(|request| {
            is_auto_thread_title_request(request)
                && request_body_contains(request, "Latest assistant reply")
    ...
    assert_eq!(turn_request_count, 1);
    assert_eq!(revision_requests.len(), 1);
    assert!(request_body_contains(
        initial_requests[0],
        "please fix the parser bug in the lexer"
    ));
    assert!(request_body_contains(
        revision_requests[0],
        "please fix the parser bug in the lexer"
