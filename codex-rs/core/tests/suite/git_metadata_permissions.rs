...
"#,
    ...
    let command = concat!(
        "test ! -e .git/config.lock && test ! -e .git/config.worktree.lock || exit 40; ",
        "git add tracked.txt && ",
    ...
    assert!(!harness.path_exists(".git/config.lock").await?);
    assert!(!harness.path_exists(".git/config.worktree.lock").await?);
