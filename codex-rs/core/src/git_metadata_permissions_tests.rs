...
async fn git_metadata_profile(
    ...
    with_git_metadata_write_access(LOCAL_FS.as_ref(), &context, profile).await
}
...
async fn linked_worktree_reopens_only_its_private_gitdir() {
    ...
    let profile = git_metadata_profile(&cwd, workspace_profile()).await;
    ...
    assert!(!policy.can_write_local_path_with_cwd(cwd.join(".git").as_path(), cwd.as_path()));
    ...
    assert!(
        policy
            .can_write_local_path_with_cwd(private_git_dir.join("index").as_path(), cwd.as_path(),)
    );
    assert!(
        policy.can_write_local_path_with_cwd(
            private_git_dir.join("logs/HEAD").as_path(),
            cwd.as_path(),
        )
    );
    ...
    assert!(
        !policy
            .can_write_local_path_with_cwd(common_git_dir.join("config").as_path(), cwd.as_path(),)
    );
    for lock in [
        common_git_dir.join("config.lock"),
        private_git_dir.join("config.worktree.lock"),
    ] {
        assert!(policy.can_write_local_path_with_cwd(lock.as_path(), cwd.as_path()));
    }
}
...
async fn read_only_project_does_not_gain_git_writes() {
    ...
    let profile = git_metadata_profile(&cwd, original.clone()).await;

    assert_eq!(profile, original);
}

#[tokio::test]
async fn executor_workspace_root_allows_git_writes_from_a_subdirectory() {
    let repo = init_repo();
    let subdirectory = repo.path().join("nested");
    fs::create_dir(&subdirectory).expect("create nested cwd");
    let cwd = PathUri::from_abs_path(&absolute(&subdirectory));
    let workspace_root = PathUri::from_abs_path(&absolute(repo.path()));
    let context = FileSystemSandboxPolicyContext {
        cwd: &cwd,
    ...
    let profile =
        with_git_metadata_write_access(LOCAL_FS.as_ref(), &context, workspace_profile()).await;

    assert!(profile.file_system_sandbox_policy().can_write_path(
        &PathUri::from_abs_path(&absolute(repo.path()).join(".git/index")),
        &context,
    ));
}
...
async fn overlay_is_recomputed_for_each_repo_cwd() {
    ...
    let first_profile = git_metadata_profile(&first_cwd, workspace_profile()).await;
    let second_profile = git_metadata_profile(&second_cwd, workspace_profile()).await;

    assert!(
        first_profile
            .file_system_sandbox_policy()
            .can_write_local_path_with_cwd(
                first_cwd.join(".git/index").as_path(),
                first_cwd.as_path(),
            )
    );
    assert!(
