...
// Do not protect config.lock or config.worktree.lock: bubblewrap creates host-side
// placeholders for missing protected files, which Git treats as active locks.
// Protect the configuration files themselves instead.
...
const PRIVATE_READ_ONLY_PATHS: &[&str] = &["config.worktree"];
...
pub(crate) async fn with_git_metadata_write_access(
    ...
    let Ok(cwd) = context.cwd.to_abs_path() else {
        return permission_profile;
    };
    let cwd_uri = context.cwd;
    let base = match fs
        .get_metadata(
            cwd_uri,
    ...
    if !file_system_policy.can_write_path(
        &PathUri::from_abs_path(&repo_root.join(WRITE_PROBE_NAME)),
        context,
    ) {
        return permission_profile;
    }
