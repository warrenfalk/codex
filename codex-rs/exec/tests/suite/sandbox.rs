...
async fn can_apply_linux_sandbox_policy(
    ...
    let Some(true_command) = find_executable_on_path("true") else {
        return false;
    };
    let spawn_result = spawn_command_under_sandbox(
        vec![true_command.to_string_lossy().into_owned()],
        command_cwd.clone(),
        permission_profile,
        sandbox_cwd,
        StdioPolicy::RedirectForShellTool,
...
if __name__ == '__main__':
    # Exercise synchronization without the forkserver's listening socket.
    context = multiprocessing.get_context("fork")
    lock = context.Lock()
    p = context.Process(target=f, args=(lock,))
    p.start()
    p.join()
