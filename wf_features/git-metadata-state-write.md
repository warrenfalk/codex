...
Configuration lock files (`config.lock` and `config.worktree.lock`) are deliberately left writable.
Protecting missing lock files can create real host-side placeholders that Git interprets as active
locks, blocking other Git processes even when the sandbox is idle. Sandbox startup must not create
these lock files; protect the configuration files themselves instead.
...
- Initially absent configuration lock files remain absent during sandbox startup and execution
  unless a command actually creates them. Direct and atomic config writes inside the sandbox still
  fail, while Git outside the sandbox can use its normal configuration lock-and-replace workflow.
- Hard-killing the sandbox supervisor does not leave other synthetic protected controls behind,
  and a later sandbox run reclaims synthetic leftovers from interruptions before cleanup supervision
  began.
