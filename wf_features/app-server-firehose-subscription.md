...
Thread-scoped notifications and server-initiated requests remain observable when the thread has zero normal subscribers. These events still reach only firehose observers and any explicit recipients; they are not broadcast to ordinary connections without a thread subscription.
...
Validation should cover protocol parsing for omitted `event/firehose` params, schema and TypeScript export generation, broadcast and targeted notification observation, thread event observation without thread subscription, notifications and pending requests with zero normal thread subscribers, exclusion of ordinary passive connections from those thread events, duplicate avoidance with multiple thread subscribers, safe wrapping of server requests, and lifecycle behavior for firehose-only connections.
